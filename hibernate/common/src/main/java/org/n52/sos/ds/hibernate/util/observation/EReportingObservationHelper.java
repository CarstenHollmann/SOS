...
public class EReportingObservationHelper {
    ...
    public Collection<NamedValue<?>> createOmParameterForEReporting(EReportingDatasetEntity series) {
        ...
        if (samplingPoint.isSetEReportingStationEntity()) {
            namedValues.add(getStation(samplingPoint.getEReportingStationEntity()));
        }
        if (samplingPoint.isSetEReportingNetworkEntity()) {
            namedValues.add(getNetwork(samplingPoint.getEReportingNetworkEntity()));
        }
        return namedValues;
    }
