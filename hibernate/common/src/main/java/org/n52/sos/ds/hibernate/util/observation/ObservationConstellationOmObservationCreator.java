...
import org.n52.sw.db.dao.AbstractFeatureDao;
...
public class ObservationConstellationOmObservationCreator extends AbstractOmObservationCreator {
    ...
    protected final ObservationConstellationEntity oc;

    protected final List<String> featureIds;

    public ObservationConstellationOmObservationCreator(ObservationConstellationEntity observationConstellation,
    ...
    public ObservationStream create() throws OwsExceptionReport, ConverterException {
        ...
        AbstractFeatureDao featureOfInterestDAO = getCreatorContext().getDaoFactory().getFeatureDao(getSession());
        for (final String featureId : getFeatureIds()) {
            final AbstractFeature feature = createFeatureOfInterest(featureOfInterestDAO.get(featureId));
        ...
        if (HibernateHelper.isNamedQuerySupported(
            ...
            Query namedQuery =
                    getSession()
                            .getNamedQuery(
                                    AbstractHibernateProcedureDescriptionGeneratorSml.SQL_QUERY_GET_UNIT_FOR_OBSERVABLE_PROPERTY_PROCEDURE_OFFERING);
            namedQuery.setParameter(ObservationConstellationEntity.OBSERVABLE_PROPERTY, oc.getObservableProperty()
                    .getIdentifier());
            namedQuery.setParameter(ObservationConstellationEntity.PROCEDURE, oc.getProcedure().getIdentifier());
            namedQuery.setParameter(ObservationConstellationEntity.OFFERING, oc.getOffering().getIdentifier());
            LOGGER.debug(
     ...
     */
    protected ObservationConstellationEntity getObservationConstellation() {
        return oc;
    }
