...
public class ObservationConstellationDao extends AbstractDao {
        ...
        for (ObservationConstellationEntity hoc : hocs) {
            ...
            if (observableProperty instanceof OmCompositePhenomenon) {
                ...
                Map<String, PhenomenonEntity> obsprop =
                        dao.getOrInsertAsMap(Lists.newArrayList(observableProperty), false);
    ...
    public void update(ObservationConstellationEntity observationConstellation, String observationType) {
        ObservationTypeEntity obsType = getDaoFactory().getObservationTypeDao(getSession()).get(observationType);
    ...
    protected Criteria getDefaultCriteria(String alias) {
        return Strings.isNullOrEmpty(alias)
                ? getSession().createCriteria(ObservationConstellationEntity.class)
                        .add(Restrictions.eq(ObservationConstellationEntity.DELETED, false))
    ...
    // default: all not deleted and published
    // transactional: all not disabled and not deleted
    // insert: update all
}
